...
def load_dataset(val_ratio):
    ...
    train_DB, valid_DB = split_train_dev(c.TRAIN_FEAT_DIR, val_ratio) #2'
...
def split_train_dev(train_feat_dir, valid_ratio):
    train_valid_DB = read_feats_structure(train_feat_dir)#3
...
def main():
    ...
    optimizer_type = 'sgd' # ex) sgd, adam, adagrad,asgd,adabound
    ...
    # Load dataset
    train_dataset, valid_dataset, n_classes = load_dataset(val_ratio)#1
    ...
    for epoch in range(start, end):
        ...
        # train for one epoch
        train_loss , train_acc = train(train_loader, model, criterion, optimizer, use_cuda, epoch, n_classes)
        
        # evaluate on validation set
        valid_loss , val_acc = validate(valid_loader, model, criterion, use_cuda, epoch)
    ...
    maxavg =avg_valid_acc.index(max(avg_valid_acc))-1
    print('Highest validation acc at epoch %d' %maxavg)
    # visualize the loss and learning rate as the network trained
    visualize_the_losses(avg_train_losses, avg_valid_losses)
    visualize_the_acc(avg_train_acc, avg_valid_acc)

def train(train_loader, model, criterion, optimizer, use_cuda, epoch, n_classes): # returns avg loss and avg accuracy
...
def validate(val_loader, model, criterion, use_cuda, epoch):# returns loss an acc
    batch_time = AverageMeter()
    losses = AverageMeter()
...
def visualize_the_acc(train_acc, valid_acc):
    # https://github.com/Bjarten/early-stopping-pytorch/blob/master/MNIST_Early_Stopping_example.ipynb
    # visualize the loss as the network trained
    fig = plt.figure(figsize=(10, 8))
